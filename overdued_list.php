...
if (!$reportView) {
  ...
  <?php
    ...
    function showOverduedList($obj_db, $array_data)
    {
        ...
        $_buffer .= '<div style="font-size: 10pt; margin-bottom: 3px;"><div id="' . $array_data[0] . 'emailStatus"></div>' . __('E-mail') . ': <a href="mailto:' . $member_d[1] . '">' . $member_d[1] . '</a> - <a class="usingAJAX btn btn-outline-danger btn-sm" style="vertical-align: middle;" href="' . MWB . 'membership/overdue_mail.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus"><i class="fas fa-envelope" aria-hidden="true"></i>&nbsp;' . __('Send Notification e-mail') . '</a> <a class="usingAJAX btn btn-success btn-sm" style="margin-left: 5px; vertical-align: middle;" href="' . MWB . 'membership/overdue_wa.php' . '" postdata="memberID=' . $array_data[0] . '" loadcontainer="' . $array_data[0] . 'emailStatus"><i class="fab fa-whatsapp" aria-hidden="true"></i>&nbsp;' . __('Send Notification Whatsapp') . '</a> - ' . __('Phone Number') . ': ' . $member_d[2] . '</div>';
